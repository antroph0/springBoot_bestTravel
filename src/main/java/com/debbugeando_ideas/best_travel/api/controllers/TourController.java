...
public class TourController {
    ...
    @Operation(summary = "Save in system a tour based in a hotel list and fly list")
    @PostMapping
    public ResponseEntity<TourResponse> post(@Valid @RequestBody TourRequest request){
       return ResponseEntity.ok(this.tourService.create(request));
    }

    @Operation(summary = "Get all tour reservations using the ID")
    @GetMapping(path = "{id}")
    public ResponseEntity<TourResponse> getAll (@PathVariable Long id){
        return ResponseEntity.ok(this.tourService.read(id));
    }

    @Operation(summary = "Delete a tour with id passed on path variable")
